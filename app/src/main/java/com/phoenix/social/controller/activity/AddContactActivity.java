...
public class AddContactActivity extends AppCompatActivity {
    ...
    private void find() {
        //获取输入的用户名称
        final String name = et_add_name.getText().toString().trim();
        ...
        Model.getInstance().getGlobalThreadPool().execute(new Runnable() {
            @Override
            public void run() {
                //去服务器判断当前查找的用户是否存在，由于没有App服务器，默认一定有此用户
    ...
    private void add() {
        Model.getInstance().getGlobalThreadPool().execute(new Runnable() {
            @Override
            public void run() {
                //去环信服务器添加好友，第二个参数是添加原因
                try {
                    EMClient.getInstance().contactManager().addContact(userInfo.getName(), "交个朋友吧~");
                    runOnUiThread(new Runnable() {
                        @Override
                        public void run() {
